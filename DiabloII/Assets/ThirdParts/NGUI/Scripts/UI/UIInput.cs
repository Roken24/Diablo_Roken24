...
#else
			if (isSelected && mEditor != null) return mEditor.text;
#endif
			return mValue;
		}
		set
...
#else
	protected int cursorPosition { get { return (isSelected && mEditor != null) ? mEditor.SelectedText.Length : value.Length; } }
#endif
	...
	bool ProcessEvent (Event ev)
	{
		...
		switch (ev.keyCode)
		{
			...
			case KeyCode.KeypadEnter:
			{
				...
				{
					char c = '\n';
					if (onValidate != null) c = onValidate(mEditor.text, mEditor.SelectedText.Length, c);
                    else if (validation != Validation.None) c = Validate(mEditor.text, mEditor.SelectedText.Length, c);
	...
	protected virtual void Append (string input)
	{
		for (int i = 0, imax = input.Length; i < imax; ++i)
		{
			...
			if (c >= ' ')
			{
				...
				if (characterLimit > 0 && mEditor.text.Length >= characterLimit) continue;
