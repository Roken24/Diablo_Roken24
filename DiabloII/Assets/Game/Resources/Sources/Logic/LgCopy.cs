...
public class LgCopy : MonoBehaviour
{
    ...
    void Start()
    {
        Transform actNode = transform.Find("Camera/Panel");
        scrollView = actNode.GetComponent<UIDraggablePanel>();

        Transform node = transform.Find("Camera/Anchor/Panel/Difficulty");
        ...
        for (int i = 0; i < 5; i++)
        {
            node = transform.Find("Camera/Anchor/Panel/Tg" + (i + 1));
        ...
        for (int i = 0; i < Config.DataLoader.sceneMaps.Count; i++)
        {
            ...
            if (s.ID % 100 != 0 && act == s.ID / 100)
            {
                if (n == 0)
                {
                    ...
                    Transform actNode = transform.Find("Camera/Anchor/Panel/ActNum");
                    UILabel actLab = actNode.GetComponent<UILabel>();
                    actLab.text = "第" + Config.CharAttribute.ConvertCHNum(act) + "幕 " + yd.name;
                }

                Transform tm = sceneCard.transform.parent.Find("Scene" + (n + 1));
                if (tm == null)
                {
                ...
                trans = tm.Find("Root/EnterButton");
    ...
    {
        Transform node = transform.Find("Camera/Anchor/Panel/Tg1");
        UIToggle tog = node.GetComponent<UIToggle>();
        if (!tog.value)
            return;
    ...
    {
        Transform node = transform.Find("Camera/Anchor/Panel/Tg2");
        UIToggle tog = node.GetComponent<UIToggle>();
        if (!tog.value)
            return;
    ...
    {
        Transform node = transform.Find("Camera/Anchor/Panel/Tg3");
        UIToggle tog = node.GetComponent<UIToggle>();
        if (!tog.value)
            return;
    ...
    {
        Transform node = transform.Find("Camera/Anchor/Panel/Tg4");
        UIToggle tog = node.GetComponent<UIToggle>();
        if (!tog.value)
            return;
    ...
    {
        Transform node = transform.Find("Camera/Anchor/Panel/Tg5");
        UIToggle tog = node.GetComponent<UIToggle>();
        if (!tog.value)
            return;
