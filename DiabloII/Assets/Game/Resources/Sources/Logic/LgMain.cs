...
public class LgMain : MonoBehaviour
{
    ...
    void Start()
    {
        Transform root = transform.Find("Camera/Anchor/Panel/Roles/A" + (int)Global.LocalHero.charactor.profession);
        root.gameObject.SetActive(true);

        root = transform.Find("Camera/Anchor/Panel/Name");
        UILabel lab = root.GetComponent<UILabel>();
        lab.text = Global.LocalHero.charactor.name;

        root = transform.Find("Camera/Anchor/Panel/Rank");
        ...
        Transform node = transform.Find("Camera/Anchor/Panel/Act");
        ...
        node = transform.Find("Camera/Anchor/Panel/Shutcut/DotAttr");
        node.gameObject.SetActive(idleDot > 0);

        node = transform.Find("Camera/Anchor/Panel/Shutcut/DotSkill");
        node.gameObject.SetActive(Global.LocalHero.charactor.level - SkillInfo.getCharDot() > 0);
        ///////////////////////////////////////////////////
        node = transform.Find("Camera/Anchor/Panel/Breakage");
