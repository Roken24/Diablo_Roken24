...
public class LgSkill : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            Transform node = transform.Find(root + "/Skill_" + (i+1));
            skillNode[i] = node;
        }
    ...
    private void UpdateSkillLevelAndDot(bool first)
    {
        ...
        UILabel dot = transform.Find("Camera/Anchor/Panel/RemDot").GetComponent<UILabel>();
        ...
        for (int i = 0; i < 3; i++)
        {
            if (first)
            {
                ...
                UILabel lab = transform.Find(str).GetComponent<UILabel>();
            ...
            for (int j = 0; j < skillInfo.Tabs[i].skills.Count; j++)
            {
                ...
                SkillTile skill = transform.Find(root + "/Skill_" + (i + 1) + "/Template" + dat.id).GetComponent<SkillTile>();
                ...
                skill.transform.Find("Sprite").gameObject.SetActive(!canAddDot);

                UILabel lvllab = skill.transform.Find("Label").GetComponent<UILabel>();
    ...
    void OnOK(params object[] args)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            for (int j = 0; j < skillInfo.Tabs[i].skills.Count; j++)
            {
                ...
                dat.tile.transform.Find("Sprite").gameObject.SetActive(!canAddDot);

                UILabel lvllab = dat.tile.transform.Find("Label").GetComponent<UILabel>();
                lvllab.text = dat.lvl.ToString();
            }
        }
